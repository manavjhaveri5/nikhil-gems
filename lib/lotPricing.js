...
/* What the kept rate is charged against. Mini hearts are costed per gram but sold
   per piece, and pricing those by the kilo asks the seller to think in a unit no
   buyer uses. So a lot can be rated by weight (the original mode, untouched) or by
   count. Everything downstream reads this rather than reaching for kg directly. */
export const LOT_RATE_BASES = [
  { mode: "lot_by_weight", unit: "kg", field: "pricePerKg", label: "kg" },
  { mode: "lot_by_count", unit: "pcs", field: "pricePerPcs", label: "pc" },
];

export const lotBasisFor = mode => LOT_RATE_BASES.find(b => b.mode === mode) || null;

/* The card's rate basis with its live numbers, or null when the card isn't on lot
   pricing at all. `qty` is what the rate multiplies; null means the card has no
   such dimension, which is what makes a pcs-only card refuse weight pricing. */
export function lotRateBasis(item) {
  const basis = lotBasisFor(item?.pricingMode);
  if (!basis) return null;
  const qty = basis.unit === "pcs" ? resolveLotPcs(item) : resolveLotKg(item);
  return { ...basis, qty, rate: num(item?.[basis.field]) };
}

export const isLotCard = item => {
  const b = lotRateBasis(item);
  return !!b && b.qty != null && (b.rate ?? 0) > 0;
};

/* Price for the whole remaining lot. Cards that aren't set up for lot pricing —
   or have no quantity to price against — fall through to today's flat behaviour
   untouched, which is what makes this safe to leave switched off. */
export function computeLotPrice(item) {
  if (!isLotCard(item)) {
    return { mode: "legacy", price: num(item?.listPrice), kg: null, pcs: null, rate: null, rateUnit: "", qty: null, belowFloor: false };
  }
  const basis = lotRateBasis(item);
  const price = round(basis.qty * basis.rate);
  const floor = num(item.lotMinPrice);
  return {
    mode: "lot",
    price,
    // kg stays on the result whatever the basis is — the description tokens and
    // the card preview have always read it.
    kg: resolveLotKg(item),
    pcs: resolveLotPcs(item),
    qty: basis.qty,
    rate: basis.rate,
    rateUnit: basis.label,
...
export function computeLotStatus(item) {
  ...
  const { price, qty, belowFloor } = computeLotPrice(item);
  if (qty == null || qty <= 0 || !price || price <= 0 || belowFloor) return "draft";
  return "active";
}
...
export function buildLotTokens(item, computed) {
  ...
  return {
    ...
    // The rate tokens only fill for the basis actually in use — a count-priced lot
    // has no price per kilo to publish, and the empty-token rule drops that line.
    price_per_kg: c.rateUnit === "kg" && c.rate != null ? String(round(c.rate)) : "",
    price_per_pcs: c.rateUnit === "pc" && c.rate != null ? String(round(c.rate)) : "",
    rate: c.rate != null ? String(round(c.rate)) : "",
...
/* Both dimensions the card is counted in, largest-first as entered. */
export function lotQtyPairs(item) {
  if (!item) return [];
  return [
...
/* The unit the lot is offered in. A card entered as 940 gm / 117 pcs is costed by
   weight but bought by the piece — nobody orders a gram of hearts — so pieces win
   the default when the card has both, and `preferUnit` overrides it either way. */
export function resolvePrimaryQty(item, preferUnit) {
  const pairs = lotQtyPairs(item);
  if (!pairs.length) return null;
  if (preferUnit) {
    const want = normalizeUnit(preferUnit);
    const hit = pairs.find(p => p.unit === want);
    if (hit) return hit;
  }
  return pairs.find(p => p.unit === "pcs") || pairs[0];
}
...
export function buildLotVariants(item, splitKeys, totalPrice, preferUnit) {
  const primary = resolvePrimaryQty(item, preferUnit);
