...
void main() {
  ...
  for (int i = 0; i < ${MAX_BANDS}; i++) {
    ...
    float w = 1.0 - smoothstep(b.y * 0.5, b.y * 0.5 + 14.0, d);
...
function parseRecipe(text) {
  ...
  return { summary: String(parsed?.summary || "").slice(0, 240), adjust, bands };
}
...
export default function PhotoEditor({ url, onSave, onClose, showToast }) {
  ...
  const initGl = useCallback(bitmap => {
    ...
    glRef.current = {
      gl, program, texture,
      uniforms: {
        texel: u("uTexel"), bandCount: u("uBandCount"),
  ...
  const draw = useCallback((values = adjust, bandList = bands, size = null) => {
  ...
  useEffect(() => {
    if (!ready) return;
    draw(showOriginal ? NEUTRAL : adjust, showOriginal ? [] : bands);
  }, [ready, adjust, bands, showOriginal, draw]);
  ...
  const save = async () => {
    ...
    try {
      ...
      draw(adjust, bands, { w: bitmap.width, h: bitmap.height });
      ...
      draw(adjust, bands, view);   // put the preview back at preview size
    } finally { setBusy(""); }
  };
  ...
  const Slider = ({ label, hint, value, min = -100, max = 100, onChange, onReset }) => (
        ...
        <div style={{ flex: 1, minHeight: 0, display: "grid", gap: 14, padding: 14, overflowY: "auto",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <button type="button" onClick={() => { setAdjust(NEUTRAL); setBands([]); setSummary(""); }}
                disabled={!touched} style={{ ...btn("transparent", C.ink), opacity: touched ? 1 : .45 }}>
                Reset
              </button>
              <span style={{ flex: 1 }} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, minWidth: 0 }}>
            ...
            {bands.length > 0 && (
              <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12, display: "grid", gap: 12 }}>
                <label style={lab}>Colour targets</label>
                {bands.map((b, i) => (
