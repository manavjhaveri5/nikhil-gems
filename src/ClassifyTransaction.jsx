      ...
      <div onMouseDown={e => e.stopPropagation()} style={inline ? {} : { width: mob ? "100%" : 500, maxWidth: "100%", height: mob ? "100%" : "auto", maxHeight: mob ? "100%" : "90vh", overflowY: "auto", background: C.bg, border: mob ? "none" : `1.5px solid ${C.border}`, borderRadius: mob ? 0 : 12, padding: mob ? "20px 16px" : "24px 26px", boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
        ...
        {(classType === "vendor_bill" || classType === "vendor_po") && <div style={{ display: "grid", gap: 12 }}>
          ...
          {classType === "vendor_po" && <div><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 6 }}><div style={{ fontSize: 10, fontWeight: 900, color: C.inkFaint, textTransform: "uppercase", letterSpacing: .65 }}>Open POs {vendorId ? `- ${vendor?.name}` : "(all vendors)"}</div>{onCreatePO && <button type="button" onClick={onCreatePO} title="Create a new Purchase Order — opens the Purchases PO form, then returns here with it selected" style={{ border: `1px solid ${C.purple}55`, background: C.purple, color: "#fff", borderRadius: 6, padding: "5px 8px", fontSize: 11, fontWeight: 800, cursor: "pointer", flexShrink: 0 }}>+ New PO</button>}</div>
            {vendorId && vendorCreditAvailable > 0.005 && <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 8, padding: "8px 11px", background: C.greenBg, border: `1px solid ${C.green}44`, borderRadius: 7, fontSize: 12 }}><span style={{ color: C.inkMid }}>Advance {vendor?.name} already holds</span><b style={{ color: C.green }}>₹{vendorCreditAvailable.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</b></div>}
            ...
            {!selectedPoId && <div style={{ marginTop: 10, padding: "10px 13px", background: vendorId ? C.blueBg : C.surface, border: `1px solid ${vendorId ? C.blue + "55" : C.border}`, borderRadius: 8, fontSize: 12, color: C.inkMid }}>{vendorId ? <>No PO selected — this records <b>₹{txnAmt.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</b> as an advance to <strong>{vendor?.name}</strong>, added to their credit and usable against any future bill.{vendorCreditAvailable > 0.005 && <> Their credit goes from ₹{vendorCreditAvailable.toLocaleString("en-IN", { minimumFractionDigits: 2 })} to <b>₹{(vendorCreditAvailable + txnAmt).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</b>.</>}</> : <>Pick a vendor above to record this as an advance to them (usable against future bills), or select a PO.</>}</div>}
          </div>}
        </div>}
